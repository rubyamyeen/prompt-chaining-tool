...
export async function getHumorFlavorSteps(humorFlavorId: number) {
  try {
    ...
    if (error) {
      console.error("[HumorFlavorSteps] Fetch error:", JSON.stringify(error, null, 2));
    ...
    console.error("[HumorFlavorSteps] Fetch exception:", msg);
...
export async function getHumorFlavorStep(id: number) {
  try {
    ...
    if (error) {
      console.error("[HumorFlavorSteps] Fetch single error:", JSON.stringify(error, null, 2));
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HumorFlavorSteps] Fetch single exception:", msg);
...
export async function createHumorFlavorStep(formData: HumorFlavorStepFormData): Promise<ActionResult> {
  try {
    ...
    if (maxIdError && maxIdError.code !== "PGRST116") {
      ...
      console.error("[HumorFlavorSteps] Max ID query error:", JSON.stringify(maxIdError, null, 2));
    ...
    if (error) {
      console.error("[HumorFlavorSteps] Create error:", JSON.stringify(error, null, 2));
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HumorFlavorSteps] Create exception:", msg);
    return { error: msg };
  }
}
...
export async function updateHumorFlavorStep(id: number, formData: Partial<HumorFlavorStepFormData>): Promise<ActionResult> {
  try {
    ...
    if (error) {
      console.error("[HumorFlavorSteps] Update error:", JSON.stringify(error, null, 2));
      return { error: error.message };
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HumorFlavorSteps] Update exception:", msg);
    return { error: msg };
  }
}
...
export async function deleteHumorFlavorStep(id: number, humorFlavorId: number): Promise<ActionResult> {
  try {
    ...
    if (error) {
      console.error("[HumorFlavorSteps] Delete error:", JSON.stringify(error, null, 2));
      return { error: error.message };
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HumorFlavorSteps] Delete exception:", msg);
    return { error: msg };
  }
}
...
export async function reorderHumorFlavorSteps(
  ...
  try {
    ...
    if (errors.length > 0) {
      console.error("[HumorFlavorSteps] Reorder errors:", JSON.stringify(errors.map(e => e.error), null, 2));
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HumorFlavorSteps] Reorder exception:", msg);
    return { error: msg };
  }
}
...
export async function getLookupData() {
  try {
    ...
    if (modelsResult.error) {
      console.error("[HumorFlavorSteps] Models lookup error:", JSON.stringify(modelsResult.error, null, 2));
    }
    if (inputTypesResult.error) {
      console.error("[HumorFlavorSteps] Input types lookup error:", JSON.stringify(inputTypesResult.error, null, 2));
    }
    if (outputTypesResult.error) {
      console.error("[HumorFlavorSteps] Output types lookup error:", JSON.stringify(outputTypesResult.error, null, 2));
    }
    if (stepTypesResult.error) {
      console.error("[HumorFlavorSteps] Step types lookup error:", JSON.stringify(stepTypesResult.error, null, 2));
    }
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error("[HumorFlavorSteps] Lookup data exception:", msg);
    return {
      llmModels: [],
      llmInputTypes: [],
      llmOutputTypes: [],
