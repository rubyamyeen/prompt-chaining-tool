...
async function getData(id: number) {
  ...
  if (flavorError || !flavor) {
    return { flavor: null, steps: [], lookups: null, error: flavorError?.message ?? "Flavor not found" };
  }
  ...
  if (stepsError) {
    console.error("[FlavorSteps] Fetch error:", stepsError);
  }
...
export default async function FlavorDetailPage({ params }: PageProps) {
  ...
  const { flavor, steps, lookups, error } = await getData(flavorId);
