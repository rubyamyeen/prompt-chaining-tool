...
async function getData() {
  ...
  return {
    flavors: flavors ?? [],
    images: images ?? [],
    errors: [flavorsError?.message, imagesError?.message].filter(Boolean),
  };
}

export default async function TestCaptionsPage() {
  const { flavors, images, errors } = await getData();
    ...
    <div className="space-y-6">
      ...
      {errors.length > 0 && (
        <div className="bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg p-4">
          ...
          {errors.map((err, i) => (
            <p key={i} className="text-red-600 dark:text-red-300 text-sm mt-1 font-mono">{err}</p>
          ))}
        </div>
      )}
