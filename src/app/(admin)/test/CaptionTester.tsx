                  ...
                  {result.stepResults.map((step, index) => (
                    ...
                    >
                      <div className="flex items-center gap-2 mb-2">
                        <span className="w-6 h-6 flex items-center justify-center bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded-full text-xs font-medium">
                          {index + 1}
                        </span>
                        <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                          {step.stepDescription || `Step ${index + 1}`}
                        </span>
                        {step.processingTimeSeconds !== undefined && (
                          <span className="text-xs text-gray-400 dark:text-gray-500 ml-auto">
                            {step.processingTimeSeconds.toFixed(2)}s
                          </span>
                        )}
                      </div>
                      <pre className="text-sm text-gray-600 dark:text-gray-400 whitespace-pre-wrap bg-gray-50 dark:bg-gray-900 rounded p-2 overflow-x-auto">
                        {step.output}
                      </pre>
                    </div>
                  ))}
